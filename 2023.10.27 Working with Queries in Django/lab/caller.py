...
def add_records_to_database():
    authors = [Author(first_name="John", last_name="Smith", birth_date="1980-05-15", nationality="American"),
    ...
    books = [Book(title="The Mystery of the Lost Key", author="John Smith", publication_year=2010, genre="Mystery",
             ...
             Book(title="Soulful Verses", author="Maria Garcia", publication_year=2015, genre="Poetry",
                  language="Spanish", page_count=150),
    ...
    reviews = [Review(reviewer_name="Alice Johnson", book_title="The Mystery of the Lost Key", author_name="John Smith",
               ...
               Review(reviewer_name="Samuel White", book_title="To Kill a Mockingbird", author_name="Harper Lee",
                      rating=5,
                      comment="An absolute classic that explores important themes with depth and compassion."),
               Review(reviewer_name="Alice Johnson", book_title="To Kill a Mockingbird", author_name="Harper Lee",
                      rating=4),
...
def find_authors_nationalities():
    ...
    result = [f"{el.first_name} {el.last_name} is {el.nationality}" for el in authors]
...
def order_books_by_year():
    ...
    result = [f"{el.publication_year} year: {el.title} by {el.author}" for el in books]
    return "\n".join(str(el) for el in result)
...
def delete_review_by_id(rev_id):
    record = Review.objects.get(id=rev_id)
    result = f"Review by {record.reviewer_name} was deleted"
    record.delete()
    return result
...
def filter_authors_by_nationalities(nationality):
    ...
    result = [el.biography if el.biography else f"{el.first_name} {el.last_name}" for el in authors]
    return "\n".join(str(el) for el in result)
...
def filter_authors_by_birth_year(start, end):
    authors = Author.objects.filter(birth_date__year__gte=start, birth_date__year__lte=end).order_by("-birth_date")
    result = [f"{el.birth_date}: {el.first_name} {el.last_name}" for el in authors]
    return "\n".join(str(el) for el in result)
...
def change_reviewer_name(old_name, new_name):
    reviews = Review.objects.filter(reviewer_name=old_name)
