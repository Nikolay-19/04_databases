...
def show_highest_rated_art():
    art = ArtworkGallery.objects.order_by("-rating").order_by("id")[0]
    return f"{art.art_name} is the highest-rated art with a {art.rating} rating!"


def bulk_create_arts(*arts):
...
# artwork3 = ArtworkGallery(artist_name="picaso", art_name="asdgf", rating=-1, price=150)
# bulk_create_arts(artwork1, artwork2, artwork3)
...
def update_operation_systems():
    # mapper = {"Asus": "Windows", "Apple": "MacOS", "Dell": "Linux", "Acer": "Linux", "Lenovo": "Chrome OS"}
    # for laptop in Laptop.objects.all():
    #     if laptop.brand in mapper.keys():
    #         laptop.brand = mapper[laptop.brand]
...
def show_hard_dungeons():
    ...
    result = [f"{el.name} is guarded by {el.boss_name} who has {el.boss_health} health points!" for el in dungeons]
...
def set_new_locations():
    Dungeon.objects.filter(recommended_level__exact=25).update(location="Enchanted Maze")
    Dungeon.objects.filter(recommended_level__exact=50).update(location="Grimstone Mines")
    Dungeon.objects.filter(recommended_level__exact=75).update(location="Shadowed Abyss")
...
# dungeon2 = Dungeon(name="Dungeon 2", boss_name="Boss 2", boss_health=500, recommended_level=25, reward="Experience", location="Crystal Caverns", difficulty="Easy")
...
def show_workouts():
    ...
    result = [f"{el.name} from {el.workout_type} type has {el.difficulty} difficulty!" for el in workouts]
    return "\n".join(str(el) for el in result)
...
# workout1 = Workout.objects.create(name="Push-Ups", workout_type="Calisthenics", duration="10 minutes", difficulty="Intermediate", calories_burned=200, instructor="Chris Heria")
# workout2 = Workout.objects.create(name="Running", workout_type="Cardio", duration="30 minutes", difficulty="High", calories_burned=400, instructor="John Smith")
# print(show_workouts())
