...
class RealEstateListing(models.Model):
    PROPERTY_TYPE_CHOICES = [('House', 'House'),
                             ('Flat', 'Flat'),
                             ('Villa', 'Villa'),
                             ('Cottage', 'Cottage'),
                             ('Studio', 'Studio')]
...
class Task(models.Model):
    ...
    def recent_completed_tasks(self, days):
        return Task.objects.filter(Q(is_completed=True) & Q(completion_date__gte=self.creation_date-timedelta(days)))
...
class Exercise(models.Model):
    ...
    def get_short_and_easy_exercises(cls):
        return cls.objects.filter(Q(duration_minutes__lt=15) & Q(difficulty_level__lt=5))

    @classmethod
    def get_exercises_within_duration(cls, min_duration, max_duration):
        return cls.objects.filter(Q(duration_minutes__gte=min_duration) & Q(duration_minutes__lte=max_duration))
